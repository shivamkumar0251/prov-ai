...
**System Providers** are managed by Prov AI. You get immediate access to models without setup, billing through your Prov AI subscription, and automatic updates when new models become available. Best for getting started quickly.
...
<Steps>
  ...
  <Step title="Test and save">
    Prov AI validates your credentials before making the provider available to your workspace.
  </Step>
</Steps>
...
<Tabs>
  ...
  <Tab title="Local (Ollama)">
    ...
    **Setup:** Install Ollama, pull models (`ollama pull llama2`), configure Prov AI connection
...
Prov AI employs a round-robin strategy for load balancing, sequentially routing model requests to each credential in the load balancing pool. If a credential hits a rate limit, it is temporarily removed from rotation for one minute to avoid futile retries.
...
System providers are billed through your Prov AI subscription with usage limits based on your plan. Custom providers bill you directly through the provider (OpenAI, Anthropic, etc.) and often provide higher rate limits.
