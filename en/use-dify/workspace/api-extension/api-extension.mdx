...
In addition to developing the corresponding module capabilities, you also need to follow the specifications below to ensure Prov AI correctly calls the API.

## API Specification

Prov AI will call your interface with the following specification:
...
When configuring an API-based Extension in Prov AI, Prov AI will send a request to the API Endpoint to verify API availability.
...
2.  Write code according to the interface specification
    ...
    async def dify_receive(data: InputData = Body(...), authorization: str = Header(None)):
        """
        Receive API query data from Prov AI.
        """
...
3.  Start the API service. The default port is 8000, the complete API address is: `http://127.0.0.1:8000/api/dify/receive`, and the configured API Key is `123456`.
    ...
    ```
4. Configure this API in Prov AI.

5. Select this API extension in the App.

When debugging the App, Prov AI will request the configured API and send the following content (example):
...
Since the Prov AI cloud version cannot access internal network API services, you can use [Ngrok](https://ngrok.com) to expose the API service endpoint to the public network to enable cloud debugging of local code. Steps:
...
This API endpoint can now be accessed publicly. At this point, we can configure this API endpoint in Prov AI for local code debugging. For configuration steps, please refer to [External Data Tool](/en/use-dify/workspace/api-extension/external-data-tool-api-extension).
