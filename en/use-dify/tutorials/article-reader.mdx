...
Create a Chatflow in Prov AI. Make sure you have added a model provider and have sufficient quota.
...
In the start node, you need to add a file variable. File upload is supported in v0.10.0 Prov AI, allowing you to add files as variable.
...
Thus, Prov AI introduced the **doc extractor** node, which can extract text from the file variable and output it as a text variable.
...
The iterative node is similar to the while loop in many programming languages, except that Prov AI has no conditional restrictions, and the **input variable can only be of type `array` (list)**. The reason is that Prov AI will execute all the content in the list until it is done.
...
In Question 1, some readers might notice that Prov AI will process all files before ending the loop, while in some cases, only a part of the files need to be operated on, not all. For this issue, you can process the file list in Prov AI using the **list operation** node. List operations can operate on all array-type variables, not just file lists.
