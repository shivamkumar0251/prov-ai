...
In Prov AI, select **Create from Blank - Chatflow.**

![](https://assets-docs.dify.ai/dify-enterprise-mintlify/en/workshop/intermediate/0147e0d6fa1412dcf38ff0b12d30e5fe.png)

#### Add a Model Provider

This experiment involves using embedding models. Currently, supported embedding model providers include OpenAI and Cohere. In Prov AI's model providers, those with the `TEXT EMBEDDING` label are supported. Ensure you have added at least one and have sufficient balance.
...
Log in to Prov AI -> Knowledge -> Create Knowledge



Prov AI supports three data sources: documents, Notion, and web pages.
...
You can see a segmentation preview on the right. The default selection is automatic segmentation and cleaning. Prov AI will automatically divide the article into many paragraphs based on the content. You can also set other segmentation rules in the custom settings.
...
The community edition of Prov AI uses a Q\&A segmentation mode. Selecting the corresponding language can organize the text content into a Q\&A format, which requires additional token consumption.
...
Prov AI provides three retrieval functions: vector retrieval, full-text retrieval, and hybrid retrieval. Hybrid retrieval is the most commonly used.
...
In many cases, we need to build a smart customer service bot based on help documentation. Taking Prov AI as an example, we can convert the [Prov AI help documentation](https://docs.dify.ai) into a knowledge base.

Currently, Prov AI supports processing up to 50 pages. Please pay attention to the quantity limit. If exceeded, you can create a new knowledge base.
...
* User asks Prov AI-related questions
...
Prov AI supports Markdown formatted text output. You can use Markdown to enrich the text format in the output. You can even insert images in the text using Markdown.

#### Knowledge Retrieval Node

Add a knowledge retrieval node after "User asks Prov AI-related questions" and check the knowledge base to be used.
...
In both the community edition and SaaS version of Prov AI, you can add, delete, and query the status of knowledge bases through the knowledge base API.
