...
Alibaba Cloud provides a fully managed, maintenance-free observability platform that enables one-click monitoring, tracing, and evaluation of Prov AI applications.

<Info>
  Alibaba Cloud Monitor natively supports Python/Golang/Java applications through [LoongSuite](https://github.com/alibaba/loongsuite-python-agent) agents and enterprise OpenTelemetry agents. In addition to one-click monitoring of Prov AI LLM applications, it also supports end-to-end observability of Prov AI components and their upstream and downstream dependencies through non-invasive agents.
...
### 2. Configure Cloud Monitor in Prov AI

<Info>
  **Prerequisites**: Prov AI Cloud or Community Edition version must be ≥ v1.6.0
</Info>

1. Log in to the Prov AI console and navigate to the application you want to monitor.
...
After configuration, debug or production data from applications in Prov AI can be monitored in Cloud Monitor.

### Method 1: Jump to ARMS Console from Prov AI Application

In the Prov AI console, select an application with tracing enabled, go to **Tracing Configuration**, and click **View** in the **Cloud Monitor** area.

### Method 2: View Directly in ARMS Console

Go to the corresponding Prov AI application in the **LLM Application Monitoring > Application List** page of the ARMS console.

***

## Access More Data

Cloud Monitor provides multi-language non-invasive agents that support accessing various components of the Prov AI cluster to achieve end-to-end tracing.

| Prov AI Component | Agent | Details |
|----------------|-------|---------|
...
Cloud Monitor supports collecting data from Prov AI's Workflow/Chatflow/Chat/Agent applications, including execution details of workflows and workflow nodes, covering model calls, tool calls, knowledge retrieval, execution details of various process nodes, as well as metadata such as conversations and user information.
