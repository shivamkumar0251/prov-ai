...
Prov AI supports connections to various external data sources. To ensure data security and access control, different data sources require appropriate authorization configurations. Prov AI provides two main authorization methods: **API Key** and **OAuth**.

## Accessing Data Source Authorization

In Prov AI, you can access data source authorization through the following two methods:
...
API Key authorization is a key-based authentication method suitable for enterprise-level services and developer tools. You need to generate API Keys from the corresponding service providers and configure them in Prov AI.
...
Prov AI provides two OAuth client configuration methods: **Default** and **Custom**.

![OAuth Client Settings](/images/knowledge-base/authorize-data-10.png)

<Tabs>
  <Tab title="Default">
    The default client is primarily supported in the SaaS version, using OAuth client parameters that are pre-configured and maintained by Prov AI. Users can add OAuth credentials with one click without additional configuration.
  </Tab>
  <Tab title="Custom">
    Custom client is supported across all versions of Prov AI. Users need to register OAuth applications on third-party platforms and obtain client parameters themselves. This is mainly suitable for data sources that don't have default configuration in the SaaS version, or when enterprises have special security compliance requirements.
  </Tab>
</Tabs>
