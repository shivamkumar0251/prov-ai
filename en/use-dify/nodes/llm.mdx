...
When using context variables from knowledge retrieval, Prov AI automatically tracks citations so users see information sources.
...
<Warning>
  Models with native JSON support handle structured outputs reliably. For others, Prov AI includes the schema in prompts, but results may vary.
</Warning>
