...
description: This document describes how to enable and use the third-party signature verification feature in the Prov AI Community Edition, including key pair generation, plugin signing and verification, and environment configuration steps, enabling administrators to securely install plugins not available on the Prov AI Marketplace.
---
...
<Warning>This feature is available only in the Prov AI Community Edition. Third-party signature verification is not currently supported on Prov AI Cloud Edition.</Warning>

Third-party signature verification allows Prov AI administrators to safely approve the installation of plugins not listed on the Prov AI Marketplace without completely disabling signature verification. This supports the following scenarios for example:

* Prov AI administrators can add a signature to a plugin sent by the developer once it has been approved.
* Plugin developers can add a signature to their plugin and publish it along with the public key for Prov AI administrators who cannot disable signature verification.

Both Prov AI administrators and plugin developers can add a signature to a plugin using a pre-generated key pair. Additionally, administrators can configure Prov AI to enforce signature verification using specific public keys during plugin installation.
...
<Warning>Keep the private key secure. If it is compromised, an attacker could add a valid signature to any plugin, which would compromise Prov AI's security.</Warning>
...
<Info>If you omit the public key argument, verification will use the Prov AI Marketplace public key. In that case, signature verification will fail for any plugin file not downloaded from the Prov AI Marketplace.</Info>

## Enabling Third-Party Signature Verification

Prov AI administrators can enforce signature verification using pre-approved public keys before installing a plugin.
...
To apply these changes, restart the Prov AI service:
