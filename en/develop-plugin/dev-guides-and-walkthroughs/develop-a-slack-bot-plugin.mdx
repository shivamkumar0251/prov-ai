...
description: This guide provides a complete walkthrough for developing a Slack Bot
  ...
  debugging, endpoint setup, verification, and packaging. You'll need the Prov AI plugin
...
> * **Slack Bot Plugin** A plugin in the Prov AI Marketplace that connects a Prov AI application with Slack. This guide focuses on how to develop that plugin.

**How It Works (A Simple Overview):**

1.  **Send a Message to the Slack Bot**

    When a user in Slack sends a message to the Bot, the Slack Bot immediately issues a webhook request to the Prov AI platform.

2.  **Forward the Message to the Slack Bot Plugin**

    The Prov AI platform triggers the Slack Bot plugin, which relays the details to the Prov AI application—similar to entering a recipient’s address in an email system. By setting up a Slack webhook address through Slack’s API and entering it in the Slack Bot plugin, you establish this connection. The plugin then processes the Slack request and sends it on to the Prov AI application, where the LLM analyzes the user’s input and generates a response.

3.  **Return the Response to Slack**

    Once the Slack Bot plugin receives the reply from the Prov AI application, it sends the LLM’s answer back through the same route to the Slack Bot. Users in Slack then see a more intelligent, interactive experience right where they’re chatting.

### Prerequisites

- **Prov AI plugin developing tool**: For more information, see [Initializing the Development Tool](/en/develop-plugin/getting-started/cli).
...
Now we’ll dive into the actual coding. Before starting, make sure you’ve read [Quick Start: Developing an Extension Plugin](/en/develop-plugin/dev-guides-and-walkthroughs/endpoint) or have already built a Prov AI plugin before.
...
For additional details on reverse-invoking Prov AI services within a plugin, see [Reverse Invocation: App](/en/develop-plugin/features-and-specs/advanced-development/reverse-invocation-app).

![Plugins permission](https://assets-docs.dify.ai/2024/12/d89a6282c5584fc43a9cadeddf09c0de.png)

#### 1.2 Edit the Configuration Form

This plugin needs to know which Prov AI app should handle the replies, as well as the Slack App token to authenticate the bot’s responses. Therefore, you’ll add these two fields to the plugin’s form.
...
*   **type**: Set to app-selector, which allows users to forward messages to a specific Prov AI app when using this plugin.
...
Go to the Prov AI platform and obtain the remote debugging address and key for your plugin.
...
Run `python -m main` to start the plugin. You should now see your plugin installed in the Workspace on Prov AI’s plugin management page. Other team members will also be able to access it.
...
From the plugin management page in Prov AI, locate the newly installed test plugin and create a new endpoint. Provide a name, a Bot token, and select the app you want to connect.
...
In your code, `self.session.app.chat.invoke` is used to call the Prov AI application, passing in parameters such as `app_id` and `query`. The response is then returned to the Slack Bot. Run `python -m main` again to restart your plugin for debugging, and check whether Slack correctly displays the Prov AI App’s reply:
...
You can now upload it to the [Prov AI Marketplace repository](https://github.com/langgenius/dify-plugins) for public release. Before publishing, ensure your plugin meets the [Publishing to Prov AI Marketplace Guidelines](/en/develop-plugin/publishing/marketplace-listing/release-to-dify-marketplace). Once approved, your code is merged into the main branch, and the plugin goes live on the [Prov AI Marketplace](https://marketplace.dify.ai/).

---

## Related Resources

- [Plugin Development Basics](/en/develop-plugin/getting-started/getting-started-dify-plugin) - Comprehensive overview of Prov AI plugin development
...
- [Reverse Invocation of Prov AI Services](/en/develop-plugin/features-and-specs/advanced-development/reverse-invocation) - Understand how to call Prov AI platform capabilities
...
- [Publishing to Prov AI Marketplace](/en/develop-plugin/publishing/marketplace-listing/release-to-dify-marketplace) - Marketplace publishing guide
- [Endpoint Detailed Definition](/en/develop-plugin/dev-guides-and-walkthroughs/endpoint) - Detailed Endpoint definition

### Further Reading

For a complete Prov AI plugin project example, visit the [GitHub repository](https://github.com/langgenius/dify-plugins). You’ll also find additional plugins with full source code and implementation details.
...
- [Reverse Invocation](/en/develop-plugin/features-and-specs/advanced-development/reverse-invocation) - Reverse-calling Prov AI capabilities
