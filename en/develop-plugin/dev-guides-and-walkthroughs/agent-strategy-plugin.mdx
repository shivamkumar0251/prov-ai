...
```bash
➜  Prov AI Plugins Developing dify plugin init
...
Tools:
    Enabled: [✔]  You can invoke tools inside Prov AI if it's enabled # Enabled by default
Models:
    Enabled: [✔]  You can invoke models inside Prov AI if it's enabled # Enabled by default
    LLM: [✔]  You can invoke LLM models inside Prov AI if it's enabled # Enabled by default
    Text Embedding: [✘]  You can invoke text embedding models inside Prov AI if it'
    Rerank: [✘]  You can invoke rerank models inside Prov AI if it's enabled
...
```
...
After finalizing the plugin’s declaration file and implementation code, run `python -m main` in the plugin directory to restart it. Next, confirm the plugin runs correctly. Prov AI offers remote debugging—go to **Plugin Management** to obtain your debug key and remote server address.
...
You can now upload it to the [Prov AI Plugins repository](https://github.com/langgenius/dify-plugins). Before doing so, ensure it meets the [Plugin Publishing Guidelines](/en/develop-plugin/publishing/marketplace-listing/release-to-dify-marketplace). Once approved, your code merges into the main branch, and the plugin automatically goes live on the [Prov AI Marketplace](https://marketplace.dify.ai/).
