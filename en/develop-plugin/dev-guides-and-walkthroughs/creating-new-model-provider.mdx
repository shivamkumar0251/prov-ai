...
* [Prov AI CLI](/en/develop-plugin/getting-started/cli)
...
Prov AI supports two model configuration methods that determine how users will interact with your provider's models:
...
The `validate_provider_credentials` method is crucial as it's called whenever a user tries to save their provider credentials in Prov AI. It should:
...
class MyProviderLargeLanguageModel(LargeLanguageModel):
    ...
    def _invoke_error_mapping(self) -> dict[type[InvokeError], list[type[Exception]]]:
        """
        Define mapping from vendor-specific exceptions to Prov AI standard exceptions.
...
1. Transform Prov AI's standardized inputs into the format required by the provider's API
2. Make the API call with proper error handling
3. Transform the API response into Prov AI's standardized output format
4. Handle both streaming and non-streaming modes

## Step 5: Debug and Test Your Plugin

Prov AI provides a remote debugging capability that allows you to test your plugin during development:

1. In your Prov AI instance, go to "Plugin Management" and click "Debug Plugin" to get your debug key and server address
...
3. Run your plugin locally with `python -m main` and test it in Prov AI
...
3. Submit a pull request to the [Prov AI official plugins repository](https://github.com/langgenius/dify-official-plugins)
...
- [Prov AI Plugin SDK Reference](https://github.com/langgenius/dify-plugin-sdks) - Look up base classes, data structures, and error types
