...
Triggers were introduced in Prov AI v1.10.0 as a new type of start node. Unlike functional nodes such as Code, Tool, or Knowledge Retrieval, the purpose of a trigger is to **convert third-party events into an input format that Prov AI can recognize and process**.

![Trigger Plugin Intro](/images/trigger_plugin_intro.PNG)

For example, if you configure Prov AI as the `new email` event receiver in Gmail, every time you receive a new email, Gmail automatically sends an event to Prov AI that can be used to trigger a workflow. However:

- Gmail's original event format is not compatible with Prov AI's input format.

- There are thousands of platforms worldwide, each with its own unique event format.

Therefore, we need trigger plugins to define and parse these events from different platforms and in various formats, and to unify them into an input format that Prov AI can accept.

## Technical Overview

Prov AI triggers are implemented based on webhooks, a widely adopted mechanism across the web. Many mainstream SaaS platforms (like GitHub, Slack, and Linear) support webhooks with comprehensive developer documentation.
...
To handle webhook events from different platforms in a unified way, Prov AI defines two core concepts: **Subscription** and **Event**.

- **Subscription**: Webhook-based event dispatch requires **registering Prov AI's network address on a third-party platform's developer console as the target server. In Prov AI, this configuration process is called a *Subscription*.**

- **Event**: A platform may send multiple types of events—such as *email received*, *email deleted*, or *email marked as read*—all of which are pushed to the registered address. A trigger plugin can handle multiple event types, with each event corresponding to a plugin trigger node in a Prov AI workflow.
...
<Note>
    For trigger plugins, the minimum required Prov AI version must be set to `1.10.0`, and the SDK version must be `>= 0.6.0`.
</Note>
...
- Some platforms (such as GitHub) support API-based webhook configuration. For these platforms, once OAuth authentication is completed, Prov AI can automatically set up the webhook.
...
For platforms like Notion, creating a subscription requires the user to manually copy the callback URL provided by Prov AI and paste it into their Notion workspace to complete the webhook setup. This process corresponds to the **Paste URL to create a new subscription** option in the Prov AI interface.
...
<Tabs>
    ...
    <Tab title="github.py">
       ...
       The `dispatch_event` method needs to determine the event type based on the request content. In the example below, this event extraction is handled by the `_dispatch_trigger_event` method.
        ...
        <Tip>
            For the complete code sample, see [Prov AI's GitHub trigger plugin](https://github.com/langgenius/dify-plugin-sdks/tree/feat/trigger/python/examples/github_trigger).
        </Tip>
...
Once an event is extracted, the corresponding implementation must filter the original HTTP request and transform it into an input format that Prov AI workflows can accept.
...
<Tabs>
    <Tab title="github.yaml">
        ...
        `subscription_constructor` is a concept abstracted by Prov AI to define how a subscription is constructed. It includes the following fields:
...
Once you have modified these two files, you'll see the **Create with API Key** option in the Prov AI interface.
...
class TriggerSubscriptionConstructor(ABC, OAuthProviderProtocol):
    ...
    def _create_subscription(
        ...
        Implementation checklist:
        1. Use the endpoint parameter provided by Prov AI
        ...
        4. Return Subscription with:
           ...
           - endpoint: The webhook endpoint URL allocated by Prov AI for receiving events, same with the endpoint parameter
           - parameters: The parameters of the subscription
           - properties: All configuration and external IDs

        Args:
            endpoint: The webhook endpoint URL allocated by Prov AI for receiving events
    ...
    def _refresh_subscription(
        ...
        1. For webhooks without expiration (e.g., GitHub):
           - Update the Subscription.expires_at=-1 then Prov AI will never call this method again
    ...
    def _fetch_parameter_options(
        ...
        and credential_type to call the external service API, then return the options to Prov AI
        for user selection.
