...
> This guide explains how to migrate from Weaviate client v3 to v4.17.0 and upgrade your Weaviate server from version 1.19.0 to 1.27.0 or higher. This migration is required for Prov AI versions that include the weaviate-client v4 upgrade.

## Overview

Starting with **Prov AI v1.9.2**, the weaviate-client has been upgraded from v3 to v4.17.0. This upgrade brings significant performance improvements and better stability, but requires **Weaviate server version 1.27.0 or higher**.

<Warning>
  **BREAKING CHANGE**: The new weaviate-client v4 is NOT backward compatible with Weaviate server versions below 1.27.0. If you are running a self-hosted Weaviate instance on version 1.19.0 or older, you must upgrade your Weaviate server before upgrading Prov AI.
</Warning>
...
- Self-hosted Prov AI users running their own Weaviate instances on versions below 1.27.0
- Users currently on Weaviate server version 1.19.0-1.26.x
- Users upgrading to Prov AI versions with weaviate-client v4

**Not affected**:

- Cloud-hosted Weaviate users (Weaviate Cloud manages the server version)
- Users already on Weaviate 1.27.0+ can upgrade Prov AI without additional steps
- Users running Prov AI's default Docker Compose setup (Weaviate version is updated automatically)
...
| Prov AI Version | Weaviate-client Version | Compatible Weaviate Server Versions |
| ------------ | ----------------------- | ----------------------------------- |
| ≤ 1.9.1      | v3.x                    | 1.19.0 - 1.26.x                     |
| ≥ 1.9.2      | v4.17.0                 | 1.27.0+ (tested up to 1.33.1)       |

<Info>
  This migration applies to any Prov AI version using weaviate-client v4.17.0 or higher.
</Info>
...
3. **Review System Requirements**
   ...
   - Verify network connectivity between Prov AI and Weaviate
...
1. **Upgrade Prov AI to a version that ships Weaviate 1.27+**:
...
2. **Run the [migration script](https://github.com/langgenius/dify-docs/blob/main/assets/migrate_weaviate_collections.py)** either locally or inside the Worker container.\
  **Option A: Run locally (if you have Python 3.11+ and dependencies installed)**:
   ...
   **Option B: Run inside Worker container (recommended for Docker setups)**:
   ...
   # Run migration script (use --no-cache for Prov AI 1.11.0+)
   ...
   <Info>
     The migration script uses environment variables for configuration, making it suitable for running inside Docker containers. For Prov AI 1.11.0+, if you encounter permission errors with `uv`, use `uv run --no-cache` instead.
   </Info>

3. **Restart Prov AI services**:
...
4. **Verify in the UI**: open Prov AI, test retrieval against your migrated knowledge bases.
...
#### Step B3: Verify in Prov AI

- Open Prov AI’s Knowledge Base UI.
...
The following new environment variable is available in Prov AI versions with weaviate-client v4:
...
### 2. Verify Prov AI Connection

Check the Prov AI logs for successful Weaviate connection:
...
1. Log into your Prov AI instance
...
```bash
# For Docker installations, ensure you're running the correct Prov AI version
docker compose pull
docker compose down
docker compose up -d
...
1. Verify API key matches in both Weaviate and Prov AI:

   ```bash
   # Check Weaviate authentication
   curl http://localhost:8080/v1/meta | jq '.authentication'

   # Check Prov AI configuration
...
2. If using anonymous access:
   ...
   Then remove `WEAVIATE_API_KEY` from Prov AI configuration.
...
3. If still slow, check network latency between Prov AI and Weaviate
...
4. Restart Prov AI to recreate schema:
   ```bash
...
### Issue: Permission Denied When Running Migration Script (Prov AI 1.11.0+)

**Cause**: The `/home/dify` directory may not exist in newer Prov AI versions, causing `uv` cache creation to fail.
...
- [Prov AI Self-Hosting Guide](/en/self-host/quick-start/docker-compose)
- [Prov AI Environment Variables](/en/self-host/configuration/environments)

### Community Resources

- [Prov AI GitHub Repository](https://github.com/langgenius/dify)
- [Prov AI GitHub Issues - Weaviate](https://github.com/langgenius/dify/issues?q=is%3Aissue+weaviate)
- [Weaviate Community Forum](https://forum.weaviate.io/)
- [Prov AI Community Forum](https://forum.dify.ai/)
...
This migration brings important improvements to Prov AI's vector storage capabilities:
...
<Info>
  If you encounter any issues not covered in this guide, please report them on the [Prov AI GitHub Issues page](https://github.com/langgenius/dify/issues) with the label "weaviate" and "migration".
</Info>
