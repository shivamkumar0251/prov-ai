...
> Before installing Prov AI, make sure your machine meets the following minimum system requirements:
> - CPU >= 2 Core
> - RAM >= 4 GiB
...
### Clone Prov AI Repository
Run the git command to clone the [Prov AI repository](https://github.com/langgenius/dify).
```Bash
...
A series of middlewares for storage (e.g. PostgreSQL / Redis / Weaviate (if not locally available)) and extended  capabilities (e.g. Prov AI's [sandbox](https://github.com/langgenius/dify-sandbox) and [plugin-daemon](https://github.com/langgenius/dify-plugin-daemon) services) are required by Prov AI backend services. Start the middlewares with Docker Compose by running these commands:
...
Access [http://127.0.0.1:3000](http://127.0.0.1:3000/)  via browsers to enjoy all the exciting features of Prov AI.
Cheers ! 🍻
