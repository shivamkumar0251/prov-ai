...
3. INLINE_COD`One-Click Install`e ins`install` is complete, find `Prov AI` in `One-Click Install` and click `install`  
...
5. After installation, enter the domain name or IP+ port s`Prov AI-characters`s step in the browser `latest`s.
- Name: application name, default `Prov AI-characters`
