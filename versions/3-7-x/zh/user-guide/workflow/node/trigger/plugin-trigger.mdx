...
1. 在工作流画布上，单击右键并选择 **添加节点** > **开始**，然后选择可用的插件触发器，或在 [Prov AI 插件市场](https://marketplace.dify.ai/?language=zh-Hans&category=trigger) 中搜索更多插件。

    <Tip>
        - 若未找到合适的触发器插件，你可以 [向社区请求](https://github.com/langgenius/dify-plugins/issues/new?template=plugin_request.yaml)、自行开发，或者改用 Webhook 触发器。

        - 一个工作流可同时拥有多个并行的插件触发器。当并行的分支连续包含相同节点时，可在相同部分之前添加变量聚合节点以合并分支，而无需在每个分支中分别重复添加相同的节点。
    </Tip>
...
Prov AI 支持通过以下两种方法创建订阅（webhook），但每个插件中的可用选项取决于插件本身的设计。

- **自动创建**：你只需选择要订阅的事件，Prov AI 将在外部系统中自动创建相应的 webhook。这需要你先通过 **OAuth** 或 **API key** 对 Prov AI 进行授权，以便其代表你配置 webhook。

- **手动创建**：你使用 Prov AI 提供的回调 URL 自行创建 webhook，无需任何授权。
...
<Tip>
    ...
    仅当关联的订阅中包含触发器对应的事件时，插件触发器才会正常工作。若选择所有可用的事件，后续添加到工作流中的任何插件触发器均可使用同一个订阅，而无需创建新订阅。
</Tip>
...
<Tabs>
    ...
    对于自托管部署，只能通过自定义 OAuth 客户端进行授权，即你需要在外部系统中自行创建 OAuth 应用。
        ...
        2. 在外部系统中，使用 Prov AI 提供的回调 URL 创建一个 OAuth 应用。

        3. 返回 Prov AI，输入新创建的 OAuth 应用的客户端 ID 和客户端密钥，然后点击 **保存并授权**。
        ...
        <Info>
            订阅配置页面上显示的 **回调 URL** 由 Prov AI 用于代表你在外部系统中创建 webhook。你无需对此 URL 执行任何操作。
    ...
    3. （可选）测试已创建的 webhook。
        ...
        <Info>
            许多外部系统在新的 webhook 创建后会自动向 Prov AI 发送 ping 请求以进行测试。
        </Info>
    
        1. 手动触发一个订阅的事件，以让外部系统向 Prov AI 提供的回调 URL 发送 HTTP 请求。

        2. 回到 **手动设置** 页面，查看底部的 **请求日志**。若 webhook 运作正常，你将看到接收到的外部请求与 Prov AI 的响应。
