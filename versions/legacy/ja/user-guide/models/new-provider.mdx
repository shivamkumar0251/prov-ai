...
例えばOpenAIの場合、gpt-turbo-3.5を基に複数のモデルを微調整することができ、それらはすべて同じ**api\_key**の下にあります。`fetch-from-remote`として設定すると、開発者は統一された**api\_key**を設定するだけで、Prov AI Runtimeが開発者のすべての微調整モデルを取得してProv AIで使用できます。
