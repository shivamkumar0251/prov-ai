...
Prov AI 是基于大语言模型的 AI 应用开发平台，初次使用时你需要先在 Prov AI 的 **设置 -- 模型供应商** 页面内添加并配置所需要的模型。

<Frame>
  <img src="/zh-cn/user-guide/.gitbook/assets/image (216).png" alt="" />
  <p>设置-模型供应商</p>
</Frame>

Prov AI 目前已支持主流的模型供应商，例如 OpenAI 的 GPT 系列、Anthropic 的 Claude 系列等。不同模型的能力表现、参数类型会不一样，你可以根据不同情景的应用需求选择你喜欢的模型供应商。**你在 Prov AI 应用以下模型能力前，应该前往不同的模型厂商官方网站获得他们的 API key 。**

### 模型类型

在 Prov AI 中，我们按模型的使用场景将模型分为以下 4 类：
...
我们为 Prov AI 云服务的用户提供了不同模型的试用额度，请在该额度耗尽前设置你自己的模型供应商，否则将会影响应用的正常使用。

* **OpenAI 托管模型试用：** 我们提供 200 次调用次数供你试用体验，可用于 GPT3.5-turbo、GPT3.5-turbo-16k、text-davinci-003 模型。

### 设置默认模型

Prov AI 在需要模型时，会根据使用场景来选择设置过的默认模型。在 `设置 > 模型供应商` 中设置默认模型。

<Frame>
  <img src="/zh-cn/user-guide/.gitbook/assets/image (113).png" alt="" />
</Frame>
...
在 Prov AI 的 `设置 > 模型供应商` 中设置要接入的模型。

<Frame>
  <img src="/zh-cn/user-guide/.gitbook/assets/image-20231210143654461 (1).png" alt="" />
</Frame>
...
在 Prov AI 中接入不同类型的模型供应商的方式稍有不同。

**接入自有模型的模型供应商**

接入自有模型的供应商后，Prov AI 会自动接入该供应商下的所有模型。

在 Prov AI 中设置对应模型供应商的 API key，即可接入该模型供应商。

<Tip>
Prov AI 使用了 [PKCS1\_OAEP](https://pycryptodome.readthedocs.io/en/latest/src/cipher/oaep.html) 来加密存储用户托管的 API 密钥，每个租户均使用了独立的密钥对进行加密，确保你的 API 密钥不被泄漏。
</Tip>
...
<Frame>
  <img src="/zh-cn/user-guide/.gitbook/assets/image (217).png" alt="" />
</Frame>