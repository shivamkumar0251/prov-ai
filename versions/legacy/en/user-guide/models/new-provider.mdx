...
For instance, with OpenAI, we can fine-tune multiple models based on gpt-turbo-3.5, all under the same **api_key**. When configured as `fetch-from-remote`, developers only need to configure a unified **api_key** to allow Prov AI Runtime to fetch all the developer's fine-tuned models and connect to Prov AI.
...
[**Adding Predefined Models**](/en-us/user-guide/models/predefined-model)**👈🏻**

For predefined models, we can connect them by simply defining a YAML file and implementing the calling code.

[**Adding Custom Models**](/en-us/user-guide/models/customizable-model) **👈🏻**
