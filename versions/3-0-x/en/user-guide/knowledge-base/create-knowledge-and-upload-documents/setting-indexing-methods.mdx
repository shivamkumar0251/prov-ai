...
2. Keyword Matching using an inverted index (a standard search engine technique). Both methods are supported in Prov AI’s knowledge base.

Both retrieval methods are supported in Prov AI’s knowledge base. The specific retrieval options available depend on the chosen indexing method.

<Tabs>
  <Tab title="High Quality">
    **High Quality**

    In the **High-Quality** Indexing Method, Prov AI offers three retrieval settings: **Vector Search, Full-Text Search, and Hybrid Search**.
