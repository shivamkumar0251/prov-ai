...
Prov AI datasets support importing from Notion and setting up **synchronization** so that data updates in Notion are automatically synced to Prov AI.
...
Next, choose a [chunking mode](sync-from-notion.md#chunking-and-cleaning) and [indexing method](../setting-indexing-methods.md) for your knowledge base, then save it and wait for the automatically processing. Prov AI not only supports importing standard Notion pages but can also consolidate and save page attributes from database-type pages.
...
After copying, go back to the Prov AI source code, and configure the relevant environment variables in the **.env** file. The environment variables are as follows:
...
Go back to the Prov AI source code, and configure the relevant environment variables in the **.env** file. The environment variables are as follows:
