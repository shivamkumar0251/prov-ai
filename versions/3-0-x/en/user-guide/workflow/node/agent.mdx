...
An Agent Node is a component in Prov AI Chatflow/Workflow that enables autonomous tool invocation. By integrating different Agent reasoning strategies, LLMs can dynamically select and execute tools at runtime, thereby performing multi-step reasoning.

## Configuration Steps

### Add the Node

In the Prov AI Chatflow/Workflow editor, drag the Agent node from the components panel onto the canvas.
...
From the dropdown menu, select the desired Agent reasoning strategy. Prov AI provides two built-in strategies, **Function Calling and ReAct**, which can be installed from the **Marketplace → Agent Strategies category**.
...
After choosing the Agent strategy, the configuration panel will display the relevant options. For the Function Calling and ReAct strategies that ship with Prov AI, the available configuration items include:
