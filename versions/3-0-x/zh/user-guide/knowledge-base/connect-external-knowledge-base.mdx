...
> 为做出区别，独立于 Prov AI 平台之外的知识库在本文内均被统称为 **“外部知识库”** 。

## 功能简介

对于内容检索有着更高要求的进阶开发者而言，Prov AI 平台内置的知识库功能和文本检索和召回机制**存在限制，无法轻易变更文本召回结果。**
...
作为中立的 LLM 应用开发平台，Prov AI 致力于给予开发者更多选择权。

**连接外部知识库**功能可以将 Prov AI 平台与外部知识库建立连接。通过 API 服务，AI 应用能够获取更多信息来源。这意味着：

* Prov AI 平台能够直接获取托管在云服务提供商知识库内的文本内容，开发者无需将内容重复搬运至 Prov AI 中的知识库；
* Prov AI 平台能够直接获取自建知识库内经算法处理后的文本内容，开发者仅需关注自建知识库的信息检索机制，并不断优化与提升信息召回的准确度。
...
<Steps>
    <Step title="建立符合要求的外部知识库 API" titleSize="h2" >
        为了确保你的外部知识库与 Prov AI 连接成功，请在建立 API 服务前仔细阅读由 Prov AI 编写的[外部知识库 API 规范](/zh-hans/guides/knowledge-base/knowledge-and-documents-maintenance/maintain-dataset-via-api)。
    </Step>
    <Step title="关联外部知识库">
        > 目前， Prov AI 连接外部知识库时仅具备检索权限，暂不支持对外部知识库进行优化与修改，开发者需自行维护外部知识库。
    ...
    <Step title="连接外部知识库">
        ...
        * **知识库名称与描述**
        * **外部知识库 API** 选择在第二步中关联的外部知识库 API；Prov AI 将通过 API 连接的方式，调用存储在外部知识库的文本内容；
...
[Prov AI 插件市场](https://marketplace.dify.ai/)内提供了 LlamaCloud 插件，可以帮助你快速连接 LlamaCloud 知识库，在 Prov AI 平台中使用由 LlamaCloud 提供的检索功能，而无需编写自定义 API。
