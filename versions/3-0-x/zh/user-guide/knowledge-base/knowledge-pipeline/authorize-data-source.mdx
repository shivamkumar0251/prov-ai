...
Prov AI 支持多种外部数据源的连接，为了确保数据安全和访问控制，不同的数据源需要进行相应的授权配置。根据数据源的特性，Prov AI 提供两种主要的授权方式：**API Key** 和 **OAuth**，以确保你能够安全地访问和导入所需的数据。

## 数据源授权界面入口

在 Prov AI 中，你可以通过以下两种方式进入数据源授权配置界面：
...
### API Key 授权
API Key 授权是一种基于密钥的身份验证方式，适用于企业级服务和开发者工具。用户需要在对应的服务提供商处生成 API Key，然后在 Prov AI 中进行配置。
...
### OAuth 授权
OAuth 是一种开放标准的授权协议，允许用户在不暴露密码的情况下，授权第三方应用访问其在特定服务提供商上的资源。Prov AI 提供两种 OAuth 授权配置方式：默认配置和自定义配置。
...
#### OAuth 客户端设置
Prov AI 提供默认和自定义两种 OAuth 客户端配置方式。

![OAuth客户端设置](/images/knowledge-base/authorize-data-10.png)

- **默认客户端**  
  仅 SaaS 版本支持默认客户端，你可以使用 Prov AI 官方预配置并维护的 OAuth 客户端参数，在 SaaS 版本一键添加 OAuth 凭据，无需进行额外的配置。

- **自定义客户端**  
  Prov AI 所有版本均支持自定义客户端。对于 SaaS 版本中尚未提供的默认配置数据源，你需要前往第三方平台自行注册并获取 OAuth 客户端参数。
