...
SearchApi 是一个强大的实时 SERP API，可提供来自 Google 搜索、Google 招聘、YouTube、Google 新闻等搜索引擎集合的结构化数据。以下是在 Prov AI 中配置和使用 SearchApi 搜索工具的步骤。
...
在 Prov AI 导航页内轻点 `工具 > SearchApi > 去授权` 填写 API Key。
