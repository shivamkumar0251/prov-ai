...
Agent 节点是 Prov AI Chatflow/Workflow 中用于实现自主工具调用的组件。它通过集成不同的 Agent 推理策略，使大语言模型能够在运行时动态选择并执行工具，从而实现多步推理。

## 配置步骤

### 添加节点

在 Prov AI Chatflow/Workflow 编辑器中，从组件栏拖拽 Agent 节点至画布。
...
从下拉菜单选择所需的 Agent 推理策略。Prov AI 内置了 **Function Calling 和 ReAct** 两种策略，可在 **Marketplace** → **Agent 策略**分类中安装使用。
...
选择 Agent 策略后，配置面板会显示对应的配置项。Prov AI 官方内置的 Function Calling 和 ReAct 策略的配置项包括：
