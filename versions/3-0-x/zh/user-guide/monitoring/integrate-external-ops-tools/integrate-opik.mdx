...
Opik 是一个企业级的 LLM 评估平台，用于评估、测试和监控大型语言模型（LLM）应用。它由 Comet 开发，旨在促进对基于 LLM 的应用程序进行更直观的协作、测试和监控。
...
在 Prov AI 应用程序中配置 Opik。打开需要监控的应用程序，在侧边菜单中打开**监控**，并在页面上选择**追踪应用性能**。
...
配置完成后，你可以照常调试或使用 Prov AI 应用程序。所有使用历史都可以在 Opik 中监控。
...
当你切换到 Opik 时，可以在仪表板中查看 Prov AI 应用程序的详细操作日志。
...
通过 Opik 的详细 LLM 操作日志将帮助你优化 Prov AI 应用程序的性能。
