...
> 在安装 Prov AI 之前，请确保你的设备符合以下最低系统要求：
> - CPU >= 2 核
> - RAM >= 4 GiB
...
运行 git 命令克隆 [Prov AI 仓库](https://github.com/langgenius/dify)。
...
Prov AI 后端服务需要一系列用于存储（如 PostgreSQL / Redis / Weaviate（如果本地不可用））和扩展能力（如 Prov AI 的 [sandbox](https://github.com/langgenius/dify-sandbox) 和 [plugin-daemon](https://github.com/langgenius/dify-plugin-daemon) 服务）的中间件。通过运行以下命令使用 Docker Compose 启动中间件：
...
通过浏览器访问 [http://127.0.0.1:3000](http://127.0.0.1:3000/) 即可享受 Prov AI 所有激动人心的功能。
干杯！🍻