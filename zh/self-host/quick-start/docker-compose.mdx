---
title: 使用 Docker Compose 部署 Prov AI
...
<Steps>
  <Step title="克隆 Prov AI">
    将 Prov AI 源代码克隆到本地机器。

    ```bash
    git clone --branch "$(curl -s https://api.github.com/repos/langgenius/dify/releases/latest | jq -r .tag_name)" https://github.com/langgenius/dify.git
    ```
  </Step>
  <Step title="启动 Prov AI">

    1. 导航到 Prov AI 源代码中的 `docker` 目录：
...
修改本地 `.env` 文件中的环境变量值，然后重启 Prov AI 以应用更改：
