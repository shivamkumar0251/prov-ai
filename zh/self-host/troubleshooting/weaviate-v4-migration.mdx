...
> 本指南说明如何从 Weaviate 客户端 v3 迁移到 v4.17.0，并将 Weaviate 服务器从 1.19.0 版本升级到 1.27.0 或更高版本。此迁移适用于包含 weaviate-client v4 升级的 Prov AI 版本。

## 概述

从 **Prov AI v1.9.2** 开始，weaviate-client 已从 v3 升级到 v4.17.0。此升级带来了显著的性能改进和更好的稳定性，但需要 **Weaviate 服务器版本 1.27.0 或更高版本**。

<Warning>
  **破坏性变更**：新的 weaviate-client v4 与 1.27.0 以下的 Weaviate 服务器版本不向后兼容。如果你正在运行 1.19.0 或更旧版本的自托管 Weaviate 实例，则必须在升级 Prov AI 之前升级 Weaviate 服务器。
</Warning>
...
- 运行低于 1.27.0 版本的自托管 Weaviate 实例的自托管 Prov AI 用户
- 当前使用 Weaviate 服务器版本 1.19.0-1.26.x 的用户
- 升级到包含 weaviate-client v4 的 Prov AI 版本的用户

**不受影响**：

- 云托管 Weaviate 用户（Weaviate Cloud 管理服务器版本）
- 已经使用 Weaviate 1.27.0+ 的用户可以直接升级 Prov AI，无需额外步骤
- 运行 Prov AI 默认 Docker Compose 设置的用户（Weaviate 版本会自动更新）
...
| Prov AI 版本    | Weaviate-client 版本 | 兼容的 Weaviate 服务器版本        |
| ------------ | ----------------------- | ----------------------------------- |
| ≤ 1.9.1     | v3.x                | 1.19.0 - 1.26.x                |
| ≥ 1.9.2     | v4.17.0             | 1.27.0+（已测试至 1.33.1）      |

<Info>
  此迁移适用于任何使用 weaviate-client v4.17.0 或更高版本的 Prov AI 版本。
</Info>
...
1. **升级 Prov AI 到包含 Weaviate 1.27+ 的版本**：
...
2. **运行[迁移脚本](https://github.com/langgenius/dify-docs/blob/main/assets/migrate_weaviate_collections.py)**，可以在本地运行或在 Worker 容器内运行。\
   ...
   **选项 B：在 Worker 容器内运行（推荐用于 Docker 设置）**：
   ...
   # 运行迁移脚本（Prov AI 1.11.0+ 请使用 --no-cache）
   ...
   <Info>
     迁移脚本使用环境变量进行配置，适合在 Docker 容器内运行。对于 Prov AI 1.11.0+，如果遇到 `uv` 的权限错误，请使用 `uv run --no-cache`。
   </Info>
...
4. **在 UI 中验证**：打开 Prov AI，针对迁移后的知识库测试检索功能。
...
以下新环境变量在包含 weaviate-client v4 的 Prov AI 版本中可用：
...
```bash
# 对于 Docker 安装，请确保运行正确的 Prov AI 版本
docker compose pull
docker compose down
docker compose up -d
...
1. 验证 Weaviate 和 Prov AI 中的 API 密钥匹配：
...
2. 如果使用匿名访问：
   ...
   然后从 Prov AI 配置中删除 `WEAVIATE_API_KEY`。
...
3. 如果仍然缓慢，请检查 Prov AI 和 Weaviate 之间的网络延迟
...
### 问题：运行迁移脚本时权限被拒绝（Prov AI 1.11.0+）

**原因**：在较新的 Prov AI 版本中 `/home/dify` 目录可能不存在，导致 `uv` 缓存创建失败。
...
- [Prov AI 自托管指南](/zh/self-host/quick-start/docker-compose)
- [Prov AI 环境变量](/zh/self-host/configuration/environments)

### 社区资源

- [Prov AI GitHub 仓库](https://github.com/langgenius/dify)
...
- [Prov AI 社区论坛](https://forum.dify.ai/)
...
<Info>
  如果你遇到本指南未涵盖的任何问题，请在 [Prov AI GitHub Issues 页面](https://github.com/langgenius/dify/issues)上报告，并添加"weaviate"和"migration"标签。
</Info>
