...
触发器是 Prov AI v1.10.0 版本中引入的一种新型起始节点。与代码、工具或知识库检索等功能节点不同，触发器的目的是**将第三方事件转换为 Prov AI 可以识别和处理的输入格式**。

![Trigger Plugin Intro](/images/trigger_plugin_intro.PNG)

例如，如果你在 Gmail 中将 Prov AI 配置为 `new email` 事件接收器，每当收到新邮件时，Gmail 会自动向 Prov AI 发送一个可用于触发工作流的事件。然而：

- Gmail 的原始事件格式与 Prov AI 的输入格式不兼容。

- 全球有数千个平台，每个平台都有其独特的事件格式。

因此，我们需要触发器插件来定义和解析来自不同平台、各种格式的事件，并将它们统一为 Prov AI 可以接受的输入格式。

## 技术概述

Prov AI 触发器基于 webhook 实现，这是一种在网络上被广泛采用的机制。许多主流 SaaS 平台（如 GitHub、Slack 和 Linear）都支持 webhook，并提供完善的开发者文档。

Webhook 可以理解为一种基于 HTTP 的事件分发器。**一旦配置了事件接收地址，这些 SaaS 平台会在订阅的事件发生时自动将事件数据推送到目标服务器。**

为了以统一的方式处理来自不同平台的 webhook 事件，Prov AI 定义了两个核心概念：**订阅（Subscription）**和**事件（Event）**。

- **订阅（Subscription）**：基于 Webhook 的事件分发需要**在第三方平台的开发者控制台中将 Prov AI 的网络地址注册为目标服务器。在 Prov AI 中，这个配置过程称为*订阅*。**

- **事件（Event）**：一个平台可能发送多种类型的事件——例如*收到邮件*、*删除邮件*或*标记邮件为已读*——所有这些都会推送到注册的地址。触发器插件可以处理多种事件类型，每个事件对应 Prov AI 工作流中的一个插件触发器节点。
...
<Note>
    对于触发器插件，Prov AI 最低版本必须设置为 `1.10.0`，SDK 版本必须 `>= 0.6.0`。
</Note>
...
- 一些平台（如 GitHub）支持基于 API 的 webhook 配置。对于这些平台，一旦完成 OAuth 认证，Prov AI 可以自动设置 webhook。

- 其他平台（如 Notion）不提供 webhook 配置 API，可能需要用户手动进行认证。

为了适应这些差异，我们将订阅过程分为两部分：**订阅构造器（Subscription Constructor）**和**订阅（Subscription）**本身。

对于像 Notion 这样的平台，创建订阅需要用户手动复制 Prov AI 提供的回调 URL，并将其粘贴到他们的 Notion 工作区以完成 webhook 设置。这个过程对应 Prov AI 界面中的**粘贴 URL 创建新订阅**选项。
...
<Tabs>
    ...
    <Tab title="github.py">
       ...
       `dispatch_event` 方法需要根据请求内容确定事件类型。在下面的示例中，事件提取由 `_dispatch_trigger_event` 方法处理。
        ...
        <Tip>
            完整代码示例，请参阅 [Prov AI 的 GitHub 触发器插件](https://github.com/langgenius/dify-plugin-sdks/tree/feat/trigger/python/examples/github_trigger)。
        </Tip>
...
一旦提取了事件，相应的实现必须过滤原始 HTTP 请求并将其转换为 Prov AI 工作流可以接受的输入格式。
...
<Tabs>
    <Tab title="github.yaml">
        ...
        `subscription_constructor` 是 Prov AI 抽象出来的一个概念，用于定义如何构造订阅。它包含以下字段：
...
修改完这两个文件后，你将在 Prov AI 界面中看到**使用 API 密钥创建**选项。
