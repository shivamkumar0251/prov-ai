---
title: '10分钟构建 Prov AI 插件指南'
description: '学习如何在10分钟内构建一个连接 Flomo 笔记服务的功能性 Prov AI 插件'
language: en
...
完成本指南后，你将创建一个 Prov AI 插件，它能够：

- 连接到 Flomo 笔记 API
- 允许用户将 AI 对话中的笔记直接保存到 Flomo
- 正确处理身份验证和错误状态
- 准备好在 Prov AI Marketplace 中分发
...
## 步骤 1：安装 Prov AI CLI 并创建项目

<Steps>
  <Step title="安装 Prov AI CLI">
    <Tabs>
      <Tab title="Mac">
        ```bash
      ...
      <Tab title="Linux">
        从 [Prov AI GitHub releases 页面](https://github.com/langgenius/dify-plugin-daemon/releases) 获取最新的 Prov AI CLI
...
<Warning>
始终优雅地处理异常并返回用户友好的错误消息。请记住，你的插件代表着你在 Prov AI 生态系统中的品牌形象。
</Warning>
...
<Steps>
  <Step title="设置调试环境">
    ...
    你可以在 Prov AI 仪表板中找到你的调试密钥和主机：点击右上角的"插件"图标，然后点击调试图标。在弹出窗口中，复制"API Key"和"Host Address"。
  </Step>
  ...
  <Step title="安装依赖并运行">
    ...
    你的插件将以调试模式连接到你的 Prov AI 实例。
  </Step>
  
  <Step title="测试功能">
    在你的 Prov AI 实例中，导航到插件并找到你的调试插件（标记为"debugging"）。
...
这将创建一个 `plugin.difypkg` 文件，你可以将其上传到 Prov AI Marketplace。
...
你已经构建了一个连接外部 API 服务的功能性 Prov AI 插件！这种相同的模式适用于与数千种服务的集成——从数据库和搜索引擎到生产力工具和自定义 API。
