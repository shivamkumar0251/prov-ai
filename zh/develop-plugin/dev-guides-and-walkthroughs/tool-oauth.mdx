...
Prov AI 中的 OAuth 涉及**两个独立的流程**，开发者需要理解并为其进行设计。

### 流程 1：OAuth 客户端设置（管理员/开发者流程）

<Note>
  在 Prov AI Cloud 上，Prov AI 团队会为热门工具插件创建 OAuth 应用并设置 OAuth 客户端，省去用户自行配置的麻烦。

  自托管 Prov AI 实例的管理员必须完成此设置流程。
</Note>

Prov AI 实例的管理员或开发者首先需要在第三方服务上将 OAuth 应用注册为受信任的应用程序。由此，他们将能够获取必要的凭证，以将 Prov AI 工具提供者配置为 OAuth 客户端。

以下是为 Prov AI 的 Gmail 工具提供者设置 OAuth 客户端的步骤示例：
...
<AccordionGroup>
  ...
  <Accordion title="在 Prov AI 中输入凭证">
  ...
  <Accordion title="授权重定向 URI">
    在 Google OAuth 客户端页面上注册 Prov AI 生成的重定向 URI：
    ...
    <Info>
      Prov AI 在 OAuth 客户端配置弹窗中显示 `redirect_uri`。它通常遵循以下格式：

      ```bash
      https://{your-dify-domain}/console/api/oauth/plugin/{plugin-id}/{provider-name}/{tool-name}/callback
      ```

      对于自托管 Prov AI，`your-dify-domain` 应与 `CONSOLE_WEB_URL` 保持一致。
    </Info>
  </Accordion>
</AccordionGroup>
...
### 流程 2：用户授权（Prov AI 用户流程）

配置 OAuth 客户端后，Prov AI 用户可以授权你的插件访问他们的个人账户。
...
提供者清单中的 `oauth_schema` 部分告诉 Prov AI 你的插件 OAuth 需要哪些凭证以及 OAuth 流程将产生什么。设置 OAuth 需要两个 schema：
...
`self.runtime.credentials` 自动提供当前用户的令牌。Prov AI 自动处理刷新。

对于同时支持 OAuth 和 API_KEY 认证的插件，你可以使用 `self.runtime.credential_type` 来区分这两种认证类型。

### 4. 指定正确的版本

早期版本的插件 SDK 和 Prov AI 不支持 OAuth 认证。因此，你需要将插件 SDK 版本设置为：

```
dify_plugin>=0.4.2,<0.5.0.
```

在 `manifest.yaml` 中，添加最低 Prov AI 版本：
