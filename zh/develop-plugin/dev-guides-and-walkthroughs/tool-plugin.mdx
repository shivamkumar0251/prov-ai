...
language: en
title: 工具插件
description: 本文档提供了如何为 Prov AI 开发工具插件的详细说明，以 Google Search 为例演示完整的工具插件开发流程。内容包括插件初始化、模板选择、工具提供者配置文件定义、添加第三方服务凭据、工具功能代码实现、调试以及打包发布。
---

<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考[英文原版](/en/develop-plugin/dev-guides-and-walkthroughs/tool-plugin)。</Note>

工具是指可以被 Chatflow / Workflow / Agent 类型应用调用的第三方服务，提供完整的 API 实现能力来增强 Prov AI 应用。例如，添加在线搜索、图像生成等额外功能。
...
关于如何准备插件开发脚手架工具的详细说明，请参考[初始化开发工具](/zh/develop-plugin/getting-started/cli)。如果你是首次开发插件，建议先阅读 [Prov AI 插件开发：Hello World 指南](/zh/develop-plugin/dev-guides-and-walkthroughs/tool-plugin)。

### 创建新项目

运行脚手架命令行工具来创建一个新的 Prov AI 插件项目。
...
插件还需要从 Prov AI 平台读取的权限。为本示例插件授予以下权限：
...
-   `parameters` 参数列表
    ...
    -   `options` 参数选项
        -   在 `llm` 模式下，Prov AI 会将所有选项传递给 LLM，LLM 可以根据这些选项进行推断。
...
完成插件开发后，需要测试插件是否能正常工作。Prov AI 提供了便捷的远程调试方法，帮助你在测试环境中快速验证插件的功能。
...
如果你想将插件发布到 Prov AI Marketplace，请确保你的插件符合[发布到 Prov AI Marketplace](/zh/develop-plugin/publishing/marketplace-listing/release-to-dify-marketplace) 中的规范。通过审核后，代码将被合并到主分支并自动上架到 [Prov AI Marketplace](https://marketplace.dify.ai/)。
...
-   [反向调用](/zh/develop-plugin/features-and-specs/advanced-development/reverse-invocation) - 反向调用 Prov AI 能力
