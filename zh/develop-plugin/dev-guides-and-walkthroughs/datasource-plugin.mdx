...
数据源插件是 Prov AI 1.9.0 中引入的一种新型插件。在知识库管道中，它们作为文档数据源和整个管道的起点。
...
- [Prov AI 插件开发：Hello World 指南](/zh/develop-plugin/dev-guides-and-walkthroughs/tool-plugin)

## **数据源插件类型**

Prov AI 支持三种类型的数据源插件：网页爬虫、在线文档和在线云盘。在实现插件代码时，提供插件功能的类必须继承自特定的数据源类。三种插件类型分别对应不同的父类。

<Info>
  要了解如何通过继承父类来实现插件功能，请参阅 [Prov AI 插件开发：Hello World 指南 - 4.4 实现工具逻辑](/zh/develop-plugin/dev-guides-and-walkthroughs/tool-plugin#4-4-implementing-tool-logic)。
</Info>
...
<Info>
  通常，数据源插件不需要使用 Prov AI 平台的其他功能，因此不需要额外的权限。
</Info>
...
- 在 `manifest.yaml` 文件中，按如下方式设置最低支持的 Prov AI 版本：

  ```yaml
  minimum_dify_version: 1.9.0
  ```
- 在 `manifest.yaml` 文件中，添加以下标签以在 Prov AI Marketplace 的数据源类别下显示插件：
...
<Info>
  有关创建提供者 YAML 文件的更多信息，请参阅 [Prov AI 插件开发：Hello World 指南 - 4.3 配置提供者凭证](/zh/develop-plugin/dev-guides-and-walkthroughs/tool-plugin#4-3-configuring-provider-credentials)。
</Info>
...
- 在你的 Prov AI 环境中导入和使用插件。
- 通过提交 pull request 将插件发布到 Prov AI Marketplace。
