...
language: en
title: 模型规范
description: 本文档详细定义了 Prov AI 模型插件开发的核心概念和结构，包括模型供应商（Provider）、AI 模型实体（AIModelEntity）、模型类型（ModelType）、配置方法（ConfigurateMethod）、模型特性（ModelFeature）、参数规则（ParameterRule）、价格配置（PriceConfig）以及各种凭据模式的详细数据结构规范。
---
