...
将 Prov AI 应用嵌入到网站中，可以直接为你的访问者带来 AI 功能。无论你想要一个浮动聊天按钮、全屏聊天界面，还是嵌入式小组件，Prov AI 都提供了适用于任何网站技术的灵活选项。
...
window.difyChatbotConfig = {
    ...
    // 可选：预填充用户上下文
    inputs: {
        name: "John Doe",          // 来自你的 Prov AI 应用的变量名
        department: "Support"
    },
...
<Steps>
  <Step title="获取你的嵌入令牌">
    在你的 Prov AI 应用中，前往 **发布 → 嵌入** 找到你的唯一令牌。
  </Step>
  
  <Step title="添加脚本">
    在你网站的 HTML 中包含配置和 Prov AI 的嵌入脚本。
  </Step>
...
为了获得更集成的体验，可以将你的 Prov AI 应用直接嵌入到网页内容中：
...
- 如果你的 Prov AI 应用需要，确保父页面通过 HTTPS 提供服务
