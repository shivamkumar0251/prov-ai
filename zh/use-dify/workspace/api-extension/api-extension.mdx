...
除了需要开发对应的模块能力，还需要遵照以下规范，以便 Prov AI 正确调用 API。

## API 规范

Prov AI 将会以以下规范调用你的接口：
...
在 Prov AI 配置 API-based Extension 时，Prov AI 将会发送一个请求至 API Endpoint，以检验 API 的可用性。
...
App 调试时，Prov AI 将请求配置的 API，并发送以下内容（范例）：
...
由于 Prov AI 云端版无法访问内网 API 服务，为了方便本地调试 API 服务，可以使用 [Ngrok](https://ngrok.com) 将 API 服务的端点暴露到公网，实现云端调试本地代码。操作步骤：
...
此 API 端点即可公网访问。至此，我们即可在 Prov AI 配置该 API 端点进行本地调试代码，配置步骤请参考 [外部数据工具](/zh/use-dify/workspace/api-extension/external-data-tool-api-extension)。
