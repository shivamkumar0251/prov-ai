...
<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考[英文原版](/en/use-dify/knowledge/create-knowledge/import-text-data/sync-from-notion)。</Note>

Prov AI 知识库支持从 Notion 导入，并支持后续的数据自动同步。
...
接下来，选择知识库的**分段设置**和**索引方式**，**保存并处理**。等待 Prov AI 自动处理数据。Prov AI 不仅可以导入 Notion 的普通类型页面，同时也支持导入并汇总保存 database 类型下的页面属性。
...
复制后回到 Prov AI 源代码下，在 **.env** 文件里配置相关环境变量，环境变量如下：
...
回到 Prov AI 源代码下，在 **.env** 文件里配置相关环境变量，环境变量如下：
