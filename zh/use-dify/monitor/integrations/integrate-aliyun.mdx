...
阿里云提供的全托管免运维可观测平台，一键开启 Prov AI 应用的监控追踪和评估。

<Info>
阿里云云监控原生支持 Python/Golang/Java 应用通过 [LoongSuite](https://github.com/alibaba/loongsuite-python-agent)
探针 & 企业级 OpenTelemetry 探针接入，在一键开启 Prov AI 大模型应用监控外，还支持通过无侵入探针对 Prov AI 组件及其上下游依赖的全链路可观测。
...
> **📌 前提条件**
> Prov AI Cloud | 社区版本号需 ≥ v1.6.0

1. 登录 **Prov AI 控制台**，并进入需要监控的 Prov AI 应用。
2. 在左侧导航栏单击 **监测**。
...
配置完成后，Prov AI 内应用的调试或生产数据可以在 **云监控** 中进行监控。

### 方式一：从 Prov AI 应用跳转 ARMS 控制台

在 Prov AI 控制台选择已开启应用追踪的应用，进入 **追踪配置**，在 **云监控** 区域单击 **查看**。

![](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9647222571/p984818.png)

### 方式二：直接在 ARMS 控制台查看

在 ARMS 控制台的 **LLM 应用监控 > 应用列表** 页面进入对应的 Prov AI 应用。

## 接入更多数据

云监控提供了多语言无侵入探针，支持接入Prov AI集群的各个组件，实现全链路追踪。
...
云监控支持采集Prov AI的Workflow/Chatflow/Chat/Agent应用，采集数据包括工作流和工作流节点的执行明细，涵盖模型调用、工具调用、知识检索和各类流程节点的执行明细数据以及会话和用户信息等元数据
