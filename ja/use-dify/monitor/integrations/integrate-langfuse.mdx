...
2. Langfuseでプロジェクトを作成します。ログイン後、ホームページで**新規**をクリックして自身のプロジェクトを作成します。この**プロジェクト**は、Prov AIの**アプリケーション**と関連付けてデータを監視するために使用されます。
...
4. Prov AIでLangfuseを設定します。監視が必要なアプリケーションを開き、サイドメニューで**モニタリング**を開き、ページ上で**アプリパフォーマンスのトレース**を選択します。
...
設定後、Prov AIのアプリケーションのデバッグまたはプロダクションデータをLangfuseで見ることができます。
...
[Langfuseプロンプト管理プラグイン](https://github.com/gao-ai-com/dify-plugin-langfuse)（コミュニティによって維持されている）は、Prov AIアプリケーションで[Langfuseで管理およびバージョン管理されているプロンプト](https://langfuse.com/docs/prompts/get-started)を使用できるようにし、大規模言語モデル（LLM）アプリケーション開発のワークフローを向上させます。主な機能は以下の通りです：
