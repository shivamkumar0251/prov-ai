...
Prov AIアプリをウェブサイトに埋め込むことで、訪問者に直接AI機能を提供できます。フローティングチャットボタン、フルスクリーンチャットインターフェース、または埋め込みウィジェットなど、Prov AIはあらゆるウェブサイト技術で動作する柔軟なオプションを提供します。
...
window.difyChatbotConfig = {
    ...
    inputs: {
        name: "John Doe",          // Prov AIアプリからの変数名
        department: "Support"
    },
...
<Steps>
  <Step title="埋め込みトークンを取得">
    Prov AIアプリで**公開 → 埋め込み**に移動して、固有のトークンを見つけます。
  </Step>
  
  <Step title="スクリプトを追加">
    ウェブサイトのHTMLに設定とProv AIの埋め込みスクリプトを含めます。
  </Step>
...
より統合された体験のために、Prov AIアプリをWebページコンテンツに直接埋め込みます：
...
- Prov AIアプリがHTTPSを要求する場合、親ページがHTTPS経由で提供されていることを確認
