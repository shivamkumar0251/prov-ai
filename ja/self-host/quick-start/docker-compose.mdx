---
title: Docker ComposeでProv AIをデプロイする
...
<Steps>
  <Step title="Prov AIをクローン">
    Prov AIのソースコードをローカルマシンにクローンします。

    ```bash
    git clone --branch "$(curl -s https://api.github.com/repos/langgenius/dify/releases/latest | jq -r .tag_name)" https://github.com/langgenius/dify.git
    ```
  </Step>
  <Step title="Prov AIを起動">

    1. Prov AIソースコード内の`docker`ディレクトリに移動します：
...
2. 管理者アカウントの設定が完了したら、以下のアドレスでProv AIにログインします：
...
ローカルの`.env`ファイルの環境変数値を変更し、Prov AIを再起動して変更を適用します：
