...
> Prov AI をインストールする前に、マシンが以下の最小システム要件を満たしていることを確認してください：
>
> * CPU >= 2 コア
> * RAM >= 4 GiB
...
git コマンドを実行して [Prov AI リポジトリ](https://github.com/langgenius/dify) をクローンします。
...
ストレージ用（PostgreSQL / Redis / Weaviate（ローカルで利用できない場合））や拡張機能用（Prov AI の [sandbox](https://github.com/langgenius/dify-sandbox) や [plugin-daemon](https://github.com/langgenius/dify-plugin-daemon) サービスなど）の一連のミドルウェアが Prov AI バックエンドサービスで必要です。次のコマンドを実行して Docker Compose でミドルウェアを起動します：
...
ブラウザで [http://127.0.0.1:3000](http://127.0.0.1:3000/) にアクセスして、Prov AI のすべての機能をお楽しみください。
乾杯！🍻