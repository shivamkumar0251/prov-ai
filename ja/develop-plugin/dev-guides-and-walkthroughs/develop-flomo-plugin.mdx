---
title: '10分でProv AIプラグインを構築するガイド'
description: 'わずか10分でFlomoメモサービスと連携する機能的なProv AIプラグインの構築方法を学びます'
language: en
...
このガイドを終えると、以下の機能を持つProv AIプラグインが完成します：

- Flomoメモ取りAPIへの接続
- AIとの会話から直接Flomoにメモを保存する機能
- 認証とエラー状態の適切な処理
- Prov AI Marketplaceでの配布準備完了
...
## ステップ1：Prov AI CLIのインストールとプロジェクト作成

<Steps>
  <Step title="Prov AI CLIのインストール">
    <Tabs>
      <Tab title="Mac">
        ```bash
      ...
      <Tab title="Linux">
        [Prov AI GitHubリリースページ](https://github.com/langgenius/dify-plugin-daemon/releases)から最新のProv AI CLIを取得してください
...
<Warning>
常に例外を適切に処理し、ユーザーフレンドリーなエラーメッセージを返すようにしてください。あなたのプラグインはProv AIエコシステムにおけるブランドを代表するものであることを忘れないでください。
</Warning>
...
<Steps>
  <Step title="デバッグ環境のセットアップ">
    ...
    デバッグキーとホストはProv AIダッシュボードで確認できます：右上の「プラグイン」アイコンをクリックし、デバッグアイコンをクリックします。ポップアップウィンドウで「APIキー」と「ホストアドレス」をコピーしてください。
  </Step>
  ...
  <Step title="依存関係のインストールと実行">
    ...
    プラグインがデバッグモードでProv AIインスタンスに接続されます。
  </Step>
  
  <Step title="機能のテスト">
    Prov AIインスタンスでプラグインに移動し、デバッグ中のプラグイン（「debugging」とマークされています）を見つけます。
...
これにより、Prov AI Marketplaceにアップロードできる`plugin.difypkg`ファイルが作成されます。
...
外部APIサービスと連携する機能的なProv AIプラグインを構築しました！この同じパターンは、データベースや検索エンジンから生産性ツールやカスタムAPIまで、何千ものサービスとの統合に使用できます。
