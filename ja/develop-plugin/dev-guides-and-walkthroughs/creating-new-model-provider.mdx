...
* [Prov AI CLI](/ja/develop-plugin/getting-started/cli)
...
Prov AI は、ユーザーがプロバイダーのモデルとどのようにやり取りするかを決定する 2 つのモデル設定方法をサポートしています：
...
`validate_provider_credentials` メソッドは、ユーザーが Prov AI でプロバイダー認証情報を保存しようとするたびに呼び出されるため、非常に重要です。このメソッドは：
...
class MyProviderLargeLanguageModel(LargeLanguageModel):
    ...
    def _invoke_error_mapping(self) -> dict[type[InvokeError], list[type[Exception]]]:
        """
        ベンダー固有の例外から Prov AI 標準の例外へのマッピングを定義します。
...
1. Prov AI の標準化された入力をプロバイダーの API が必要とする形式に変換
2. 適切なエラー処理で API 呼び出しを実行
3. API レスポンスを Prov AI の標準化された出力形式に変換
4. ストリーミングモードと非ストリーミングモードの両方を処理

## ステップ 5: プラグインのデバッグとテスト

Prov AI は、開発中にプラグインをテストできるリモートデバッグ機能を提供しています：

1. Prov AI インスタンスで「プラグイン管理」に移動し、「プラグインをデバッグ」をクリックしてデバッグキーとサーバーアドレスを取得
...
3. `python -m main` でプラグインをローカルで実行し、Prov AI でテスト
...
3. [Prov AI 公式プラグインリポジトリ](https://github.com/langgenius/dify-official-plugins)にプルリクエストを提出
...
- [Prov AI プラグイン SDK リファレンス](https://github.com/langgenius/dify-plugin-sdks) - 基底クラス、データ構造、エラータイプを参照
