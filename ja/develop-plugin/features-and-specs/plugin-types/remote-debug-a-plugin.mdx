...
language: en
title: プラグインのデバッグ
description: このドキュメントでは、Prov AIのリモートデバッグ機能を使用してプラグインをテストする方法を紹介します。デバッグ情報の取得、環境変数ファイルの設定、プラグインのリモートデバッグの開始、プラグインのインストール状態の確認について詳細な手順を提供します。この方法により、開発者はローカルで開発しながら、Prov AI環境でリアルタイムにプラグインをテストできます。
---
...
プラグイン開発が完了したら、次のステップはプラグインが正常に機能するかどうかをテストすることです。Prov AIは、テスト環境でプラグインの機能を素早く検証するための便利なリモートデバッグ方法を提供しています。
