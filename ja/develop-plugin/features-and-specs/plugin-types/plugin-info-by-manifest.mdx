...
<ParamField path="privacy" type="string">
  プラグインのプライバシーポリシーファイルの相対パスまたはURLを指定します。例：`"./privacy.md"`または`"https://your-web/privacy"`。プラグインをProv AI Marketplaceに掲載する予定がある場合、明確なユーザーデータの使用とプライバシーに関する声明を提供するために**このフィールドは必須**です。詳細な記入ガイドラインについては、[プラグインプライバシーデータ保護ガイドライン](/ja/develop-plugin/publishing/standards/privacy-protection-guidelines)を参照してください。
</ParamField>
